  ...
  return (
    <section className="modeling-outliner" aria-label="Scene outliner">
      ...
      <div className="modeling-outliner-tree">
        {filteredNodes.map((node) => <div key={node.id} className={`modeling-outliner-node ${selectedNodeId === node.id ? 'is-selected' : ''}`} role="treeitem" aria-selected={selectedNodeId === node.id}>
          <button type="button" className="modeling-outliner-select" onClick={() => onSelect(node.id)}><ChevronDown size={12} className="modeling-node-chevron" /><GitBranch size={13} /><span>{node.name}</span></button>
