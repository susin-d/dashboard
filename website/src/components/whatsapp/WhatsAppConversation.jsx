...
export function WhatsAppConversation({
  ...
  const participantInfoMap = useMemo(() => {
    ...
    // 2. Seed from all loaded chats and contacts
  ...
  const participantNameMap = useMemo(() => {
    return participantInfoMap.nameMap
  }, [participantInfoMap])

  const resolveReactionSender = (rx) => {
    if (!rx) return { name: 'Contact', avatar: null, initial: '?' }

    // 1. Check if reaction was sent by current user
    const isMe =
      rx.sender === 'You' ||
      rx.sender === 'me' ||
      rx.is_from_me ||
    ...
    // 2. Check if reaction object has an explicit valid human sender_name
    ...
    if (
      ...
      return {
        ...
        isMe: false,
        avatar: avatar,
        initial: getSenderInitial(explicitName),
      }
    }

    // 3. Look up sender in participantInfoMap
    ...
    if (rawSender && participantInfoMap.nameMap.has(rawSender)) {
      ...
      return {
        name,
        isMe: false,
        avatar: participantInfoMap.avatarMap.get(rawSender) || null,
        initial: getSenderInitial(name),
      }
    }
    if (cleanSender && participantInfoMap.nameMap.has(cleanSender)) {
      ...
      return {
        name,
        isMe: false,
        avatar: participantInfoMap.avatarMap.get(cleanSender) || null,
        initial: getSenderInitial(name),
      }
    }
    ...
    // 4. In a 1-on-1 chat, any incoming reaction is from the contact
    if (!chat?.is_group && chat?.name && chat.name !== 'Contact' && chat.name !== chat.id) {
      return {
        name: chat.name,
        isMe: false,
        avatar: chat.avatar_url || null,
        initial: getSenderInitial(chat.name),
      }
    }
    ...
    if (
      ...
      return {
        ...
        isMe: false,
        avatar: null,
        initial: getSenderInitial(cleanSender),
      }
    }
                    ...
                    return (
                      <div className="whatsapp-reactions-badge-list">
                        <div
                          ...
                          title={(msg.reactions || [])
                            .map((r) => `${resolveReactionSender(r).name}: ${r.emoji}`)
                            .join('\n')}
                        >
                ...
                {Array.from(new Set(reactionModalData.reactions.map((r) => r.emoji))).map((emoji) => {
                  ...
                  return (
                    <button
                      key={emoji}
                      type="button"
                      className={`whatsapp-reaction-tab ${isActive ? 'active' : ''}`}
                  ...
                  return (
                    <div key={idx} className="whatsapp-reaction-user-row">
                      <div className="whatsapp-avatar small">
                        ...
                        <div
                          className="whatsapp-avatar-fallback"
                          style={senderInfo.avatar ? { display: 'none' } : {}}
                        >
