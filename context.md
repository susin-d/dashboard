...
> **Last updated:** 2026-08-25 (Eve chat full-height viewport fix — `.content:has(.eve-page-container)` in `layout-symmetry.css` and `.eve-page-container` / `.eve-chat-section` in `eve.css` now fill the full viewport height (`calc(100vh - 64px)` with `flex: 1` and `min-height: 0`), pinning the composer cleanly at the bottom and eliminating dead empty space below the chat card; lint/build OK)
> **Also 2026-08-25:** (Studio Plan/Build mode & Model selector + Eve stream caret fix — replaced the old template dropdown in `StudioHero.jsx` with a Mode selector (`Plan` mode for interactive planning & questions vs `Build` mode for generating architecture and writing code in one go) and an AI Model selector (`GPT-5 mini`, `Gpt oss:120b`, `Claude 3.5 Sonnet`, `Gemini 1.5 Flash`, `Llama 3.1`); brief instructions handed to `BuilderChat` to execute in the chosen mode; removed the isolated black block cursor (`.eve-stream-caret`) in Eve Chat; lint/build OK)
