...
> **Last updated:** 2026-08-24 (Eve voice: OpenRouter Fish S2.1 Pro Free TTS — `fish-audio/s2.1-pro-free:free` via `POST /audio/speech` using `OPENROUTER_API_KEY`; provider `openrouter` alongside browser/google; user choice in `settings/eve-speech` + Settings EveVoiceSection preview + `useEveVoice` server playback)
