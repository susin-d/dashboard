...
> **Last updated:** 2026-08-25 (Eve tool calling robustness & timezone fix — (1) fixed `AttributeError` on `tc.name` for OpenAI-compatible client non-streaming tool call extraction in `openai_compat.py`; (2) added `RESOURCE_ALIASES` normalizer for workspace record tools (`list/create/update`) in `handlers/workspace.py` and convenience tool aliases (`list_jobs`, `list_todos`, `list_projects`, etc.) in `dispatcher.py` with graceful error returns instead of raising `ValueError`; (3) fixed `expire_stale_ringing` in `CallRepository` to ensure `updated_dt` is offset-aware before datetime threshold comparison; 137 tests passing)
