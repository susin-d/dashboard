...
- **Hooks:** `hooks/` (`useAuth`, `useRouter`, `useThemeCustomizer`, `useWorkspaceData`, `useCustomUI` (Eve UI runtime `<style>` injection + `eve-ui-update` event + per-page), `call/` `useWebRTC`/`useEveVoice`/`callConstants`/`callHelpers`/`useCallCenter`) + `usePersistentState`, `useLocalNotifications`.
- **API clients** `lib/` — one per backend feature, all via `request.js` `apiRequest` (dedup + 30s cache + 429/502 retry). Split `workspaceApi/` when grows. `uiPreferencesApi` (`get/update/reset/history`), `firebase.js`, `authApi.js`.
- **Pages:** Dashboard, Projects (`projects/` package), ProjectDetail, Jobs (timeline chart), Hackathons, Todo, Documents, Workspace (IDE + Eve Agent panel + Browser), Studio (`studio/` hero `StudioHero`+`studioBrief` → builder `StudioBuilderPage`, apps `StudioAppsPage`, templates), Eve (chat+memory+voice+schedules), Calls (WebRTC+Twilio PSTN), WhatsApp (`conversation/` package), Mails (Gmail tabs), Calendar, Contacts (`contacts/` package), CompetitiveCoding, Stats, Settings (`settings/` sections incl. `AppearanceSection` Eve UI), `CustomPage` (`/custom/<slug>` via Eve `create_custom_page`), Themes, Profile, Onboarding, Landing (`landing/` Framer Motion cinema), etc.
- **Config:** `config/navigation.js`, `config/search/` (7 modules incl. Eve UI Appearance entry), `dashboard/dashboardConfig.js`, `themes/` 22 presets, `utils/` pure transformers, `styles/` tokens→base→utilities→responsive→components (`eve-ui.css`)→pages→`layout-symmetry.css`.
...
- Calls: `in_app` (WebRTC) vs Twilio PSTN (`TWILIO_*`, `provider/external_sid/phone_number`, `build_eve_twiml/build_human_twiml`, `useCallCenter dial(provider,phone)` + barge-in `interruptEve`, `CallRepository.update_status` terminal guard).
