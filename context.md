...
> **Last updated:** 2026-08-25 (Studio builder polish — removed the dot from the `Preview` tab toggle button and removed the bottom Git panel from the Code explorer sidebar, keeping the workspace clean and streamlined; lint/build OK)
> **Also 2026-08-25:** (Studio builder redesign — transformed into Google AI Studio 2-column layout: top navigation bar with `< Back to start`, project title, `Remix`, `Share`, `Publish`, and terminal toggle; left column AI assistant chat with model indicator, checkpoint cards, plan approval, and floating composer (`Make changes, add new features, ask for anything…`, attachments, voice mic, and circular submit arrow); right column main stage with segmented `Preview` | `Code` toggle tabs, route address bar (`◫ /`), reload/fullscreen controls, full-bleed interactive preview canvas, Monaco code editor with file explorer, and dockable bottom terminal console; strictly monochrome styling; lint/build OK)
