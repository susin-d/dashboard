...
Last updated: 2026-09-11 — Provider reads, contests, hackathons, and notifications use cache-first snapshots; request logging is middleware-safe; EveComposer, avatar viewport updates, and VRM fallback animation are fixed; Avatar Studio now has compact sizing, stronger route-local surfaces, and centralized paint color defaults.
...
- **Layering:** Routes → Services/Repos → Core/Models. Never import FastAPI types in Services/Repos. Use `CurrentUser`/`CurrentUserId`/`DbClient` from `core/dependencies.py`.
 ...
 - **Performance:** hot reads `async+to_thread`, composite indexes, pools `5/5 recycle 300`, Redis/LRU `cached` per-user + stale-while-revalidate snapshots in `core/cache.py`, workspace disk. Provider-backed GitHub, Calendar, Drive, Gmail, coding stats, contests, hackathons, and dashboard reads return snapshots/empty states while refreshes run in the background. `usage:summary/logs` `SHORT 30s` + invalidates. Rate-limit `10r/s burst 60` + CORS via `$cors_allow_*` + `RateLimitMiddleware`.
...
## 6. Current snapshot
  ...
  - Avatar modeling workspace (ADR 0034, 0039, 0040): editor shell under `pages/avatar-studio/` with schema-v2 normalized GLB/GLTF/VRM/OBJ/FBX scenes, `/modeling/projects` filesystem API, outliner, inspector, timeline, UV painting, GLB export, Eve events. Route-local editor sizing and crimson surface contrast are tuned for the reference layout; paint settings use the shared avatar default color constant.
  - GET caching: `core/cache.py` `cached` per-user keys (Redis/LRU) for hot GETs plus Redis/local stale-while-revalidate snapshots, refresh deduplication, and request timing headers.
