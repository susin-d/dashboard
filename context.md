...
> **Last updated:** 2026-08-25 (Studio Apps page — new `studio-apps` route + sidebar item under the Studio group listing finished apps (`build_status: ready`) with Open Builder + Run App; Eve `WORKSPACE_PAGES` + search palette entry. Previous: <1s Eve voice: `POST /eve/voice/stream` fast path — groq `llama-3.1-8b-instant` (`GROQ_VOICE_MODEL`) streaming, sentence-chunked TTS frames, no RAG/tools; Twilio `/gather-fast`; frontend `streamEveVoice` sequential chunk playback with blocking fallback; measured 462ms first audio mock)
