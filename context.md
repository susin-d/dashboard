...
> **Last updated:** 2026-08-25 (Studio builder toolbar controls — integrated mobile/web size changer (`MonitorSmartphone` button inside `aistudio-route-bar` that toggles between full-width desktop view and framed 390px mobile viewport), live page reload (`RefreshCw` icon), and fullscreen canvas/site toggle (`Maximize2`/`Minimize2`); lint/build OK)
> **Also 2026-08-25:** (Studio composer scroll fix — wrapped textarea and toolbar inside `.builder-composer-box` with `border-radius: 18px` and `overflow: hidden`; textarea is now borderless with custom 4px scrollbars contained strictly inside the box and above the toolbar, preventing scrollbar bleed across rounded borders; lint/build OK)
