...
> **Last updated:** 2026-08-25 (Eve queued messages theme fix — removed hardcoded dark styling on `.eve-queued-panel`; panel, header, grip, row items, and action pills now strictly use design tokens (`var(--bg-card)`, `var(--bg-secondary)`, `var(--text-primary)`, `var(--border-color)`), rendering with seamless theme parity in both light and dark modes; lint/build OK)
> **Also 2026-08-25:** (Eve chat full-height viewport fix — `.content:has(.eve-page-container)` in `layout-symmetry.css` and `.eve-page-container` / `.eve-chat-section` in `eve.css` now fill the full viewport height (`calc(100vh - 64px)` with `flex: 1` and `min-height: 0`), pinning the composer cleanly at the bottom and eliminating dead empty space below the chat card; lint/build OK)
