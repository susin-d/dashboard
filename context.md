...
> **Last updated:** 2026-08-26 (Full backend test suite — migrated server/tests from unittest to **pytest** (486 tests passing): new layout tests/{unit,api,services,e2e} + shared scaffolding in tests/support/ + root conftest.py that blocks .env loading and points DATABASE_URL at a throwaway SQLite so tests run the REAL stack. 11 production bugs found & fixed by the new suite: mixins PEP-649 breakage; account-deletion 500; documents/hackathons/notifications/eve_schedules silent field loss in SQL handlers (eve_schedules were never even registered — automated schedules never executed; collection-group fan-out added to SqlClient); PATCH-on-missing phantom creates; RequestValidationError handler crash on pydantic v2 ctx.error; mark_all_read Firestore reference; hackathon manual-source marker clobbered; workspace-record KeyError robustness. Verification command repo-wide: python -m pytest tests -q)
