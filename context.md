...
| Features        | `documents`, `todos`, `contacts`, `profiles`, `notifications`, `email`, `eve`, `eve_stream`, `calls` | EVE = AI assistant (`eve` non-streaming + `eve_stream` SSE `POST /eve/chat/stream`);`calls` = WebRTC signaling; `contacts` = Address book / contacts directory                 |
