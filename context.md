...
> **Last updated:** 2026-08-25 (Talk-over edge cases: in-app barge-in - `useEveVoice.interruptEve` aborts stream+drains audio queue+cancels TTS, hold-to-talk press cuts Eve off, new CircleStop Interrupt button on CallScreen, newer speech supersedes in-flight turn via AbortController (no silent drop); server `CallRepository.update_status` terminal guard (`declined/ended/missed` final - fixes missed-vs-accept race), relay sends hardened vs mid-turn hangup; tests 131->137 OK)
