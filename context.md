...
> **Last updated:** 2026-08-26 (Security Phase 4 — BOLA per-entity wiring + SSRF 0.0.0.0 + nginx/relay: `sql/{todos,projects,jobs,contacts,documents,hackathons,eve,calls,whatsapp,notifications}` now enforce `PermissionError` owner check + immutable `id/user_id/created_at/deleted` + per-model allowlists + user-scoped `delete_*_doc(user_id)`; `http_requests.py` adds `is_unspecified/is_multicast/not is_global` + redirect `location` check + `follow_redirects=False`; `website/nginx.conf` HSTS/CSP/Permissions-Policy sync; `twilio_relay.py` 4096 size cap; prior: Twilio hard enforce, docs prod-gate, worker HMAC, RLS `starwaves_app`, `SECURITY.md`, rate-limit, `pickle→json`, SSRF, CORS, HSTS/CSP, preview, OAuth, `realpath`, `DOMPurify`.
