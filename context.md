...
> **Last updated:** 2026-08-25 (Dual call: in_app WebRTC + Twilio PSTN — `TWILIO_*` config, `calls` provider/external_sid/phone_number columns + `_ensure_call_provider_columns`, `services/twilio` client/TwiML + `calls_twilio` routes + `eve/handlers/call` twilio branch + `callsApi`/`useCallCenter` dual; reusable systems: `core/errors|http|pagination|dependencies|ws/base` etc.)
