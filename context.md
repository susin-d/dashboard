...
> **Last updated:** 2026-08-26 (Security Phase 2 — rate-limit + Twilio verify + RLS + pickle→json: `core/rate_limit.py` token-bucket (Redis/in-memory, 10/min auth, 20/min eve, 100/min workspace, 10/min cron, prod-only) + `main.py` wired; `core/cache.py` `pickle`→`json` + empty-prefix guard; `services/twilio/verify.py` `X-Twilio-Signature` HMAC-SHA1 `base64` check (soft-enforce `calls_twilio.py` gather/status/gather-fast); `sql/migrations.sql` RLS `user_isolation_*` policies for `todos/projects/documents` + `server/app/db/session.py` note; Phase 1 already: fail-closed `AUTH_SECRET_KEY/CRON_SECRET`, `password.py` `secrets` + `hmac`, `cron` strict HMAC, `webhook` `WHATSAPP_WORKER_SECRET`, `shell=False` delegation, SSRF `_assert_public_url`, CORS `*.vercel.app` removal + HSTS/CSP, preview `allow-same-origin` removal, OAuth validated `targetOrigin`, `sql/base` BOLA guards, `workspace_files` realpath, `whatsapp` typed payloads, `MailsPage` `DOMPurify`, Docker/git secrets.
