...
Last updated: 2026-08-31 — CORS/rate-limit fix: nginx burst 30→60 (preflight double), vercel regex + wildcard in cors.py, conditional CORS maps (fix 429 No Allow-Origin)
...
- Landing: sharp Linear cinema (`LandingPage.jsx` 87L + 8 Framer Motion sections + `cinema.css` scoped 373L, fixed dark #000).
  - Security: RLS `SET LOCAL` via `set_rls_user` in `sql/base` generic handlers, `starwaves_app` role, `SECURITY.md`, rate-limit 10r/s burst 60 (was 30, 429 had No Allow-Origin → now conditional maps + vercel regex, 443 server inherits http-level to avoid override) + `RateLimitMiddleware` `Retry-After`, `pickle→json`, CORS allowlist (susindran.in+vercel.app+localhost+capacitor, wildcard `*.vercel.app` via regex), `realpath`, `DOMPurify`, pip `no-build-isolation` + npm `--ignore-scripts`, UI CSS sanitized (blocks `@import`/`javascript:`/external `url`/`< >`) + allowlisted tokens + history-capped 20.
  - Infra: compose lean e2-micro (pgvector 128M, redis 96M, server 512M), Nginx 10r/s burst 60 + Gzip (now `$cors_allow_origin`+`$cors_allow_*` conditional maps + `burst 60` for /api /ws + 443 inherits http-level), Vercel cron `/cron/execute-schedules`.
