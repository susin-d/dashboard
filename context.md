...
> **Last updated:** 2026-08-25 (Eve chat spacing fix — removed `min-height: 480px` from `.eve-chat-section` and `min-height: 320px`, `max-height: 600px`, `flex: 1 1 auto` from `.eve-messages-feed`; feed is now `flex: 0 0 auto` so the quick-prompt cards and the composer/input box sit immediately adjacent with no dead whitespace gap between them)
> **Also 2026-08-25:** (Studio landing page redesign — `StudioProjectsPage` transformed from a blank single-prompt void into a rich, sleek full-stack creation hub: new `StudioHero.jsx` with AI badge, refined prompt card, interactive quick-prompt suggestion chips; new `StudioStarterTemplates.jsx` and `StudioWorkflowFeatures.jsx`; lint/build OK)
