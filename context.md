...
> **Last updated:** 2026-08-21 (Eve streaming: `POST /eve/chat/stream` SSE endpoint with per-provider streaming (`call_stream` on OpenAI Responses / Anthropic Messages / Gemini / OpenAI-compatible), streamed tool loop, session auto-create on done; frontend streams deltas with typing dots -> caret -> tool chip -> Stop button and auto-fallback to `/eve/chat`; nginx SSE location buffering off)
...
| Features        | `documents`, `todos`, `contacts`, `profiles`, `notifications`, `email`, `eve`, `eve_stream`, `calls` | EVE = AI assistant (`eve_stream` = SSE chat stream);`calls` = WebRTC signaling; `contacts` = Address book / contacts directory                 |
