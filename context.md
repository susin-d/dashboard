...
> **Last updated:** 2026-08-25 (Studio Plan/Build mode & Model selector + Eve stream caret fix — replaced the old template dropdown in `StudioHero.jsx` with a Mode selector (`Plan` mode for interactive planning & questions vs `Build` mode for generating architecture and writing code in one go) and an AI Model selector (`GPT-5 mini`, `Gpt oss:120b`, `Claude 3.5 Sonnet`, `Gemini 1.5 Flash`, `Llama 3.1`); brief instructions handed to `BuilderChat` to execute in the chosen mode; removed the isolated black block cursor (`.eve-stream-caret`) in Eve Chat; lint/build OK)
> **Also 2026-08-25:** (Studio Preview & Code tab fixes — resolved preview iframe URL against `API_URL` to avoid Vite SPA fallback; backend `_resolve_preview_file` now falls back to any root `.html` file (e.g. `hello.html`) or friendly placeholder; `BuilderChat` invokes `onAssistantReply` after Eve messages to auto-refresh the file tree and preview; `StudioBuilderPage` `handleSelectCodeTab` auto-opens project entry files when switching to the Code tab; 137 backend tests + frontend lint/build OK)
