...
> **Last updated:** 2026-08-25 (Studio composer scroll fix — wrapped textarea and toolbar inside `.builder-composer-box` with `border-radius: 18px` and `overflow: hidden`; textarea is now borderless with custom 4px scrollbars contained strictly inside the box and above the toolbar, preventing scrollbar bleed across rounded borders; lint/build OK)
> **Also 2026-08-25:** (Studio builder polish — removed the dot from the `Preview` tab toggle button and removed the bottom Git panel from the Code explorer sidebar, keeping the workspace clean and streamlined; lint/build OK)
