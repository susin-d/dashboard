...
- **Gestures** (10% per wheel notch — option A, matches common design tools):
  ...
  - **Wheel zoom** — `1.1×` / `1÷1.1×` per notch, clamped `[0.4, 4]`. Live2D adjusts `panRef` so the world point under the cursor stays under the cursor; VRM scales `camera.position.z` on top of the existing `zoom` slider.
  - **Double-click reset** — clears `userPan` / `userZoom` to defaults and persists.
...
- **Persistence**: two new keys in the `prefs` JSON blob — `userPan: { x, y }` and `userZoom: number`. The blob is already round-tripped by `saveAvatarPreferences` (no schema migration). Defaults live in `AVATAR_DEFAULTS`.
- **Constants / clampers**: `PAN_MIN/MAX ±1000`, `USER_ZOOM_MIN 0.4`, `USER_ZOOM_MAX 4`, plus `clampUserPan` / `clampUserZoom` mirroring existing `clampScale` / `clampZoom`.
- **Wiring**: `EveAvatar` reads `prefs.userPan` / `prefs.userZoom`, forwards to both renderers, exposes `onTransformChange(pan, zoom)` to the parent. `AvatarPage` debounces 350 ms and persists via `saveAvatarPreferences`. A small HUD hint reads `Drag to pan · Scroll to zoom · Double-click to reset`.
...
- **Negative / Cost:** two new persisted keys (small); wheel handlers must call `preventDefault` to avoid page scroll; refactored pointer handlers in `VrmModel` (drag-to-orbit removed — `autoRotate` still turntables if enabled).
...
- `website/src/components/eve/avatar/avatarConstants.js` (`clampUserPan`, `clampUserZoom`, `AVATAR_LIMITS.PAN_MIN/MAX`, `AVATAR_LIMITS.USER_ZOOM_MIN/MAX`)
