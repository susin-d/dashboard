...
def build_eve_twiml(message: str, voice: str = "alice", language: str = "en-US", gather: bool = True) -> str:
    ...
    safe = _escape(message[:1600] or DEFAULT_CALL_GREETING)
    if gather:
...
<Response>
    ...
    <Gather input="speech" speechTimeout="auto" language="{language}" action="/api/v1/calls/twilio/gather-fast" method="POST">
        <Say voice="{voice}" language="{language}">{EVE_SPEAK_AFTER_TONE}</Say>
    </Gather>
    <Say voice="{voice}" language="{language}">{EVE_GOODBYE_NO_SPEECH}</Say>
    <Hangup/>
</Response>"""
    return f"""<?xml version="1.0" encoding="UTF-8"?>
<Response>
...
def build_echo_twiml(text: str) -> str:
    ...
    return f"""<?xml version="1.0" encoding="UTF-8"?>
<Response>
    <Say voice="alice">{EVE_ECHO_PREFIX}{safe}</Say>
    <Hangup/>
</Response>"""
