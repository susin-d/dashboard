...
async def google_calendar_callback(
    ...
    try:
        ...
        return HTMLResponse(
            ...
            else {{ window.location.href = "{settings.frontend_url}/app/setting?calendar=error&reason={reason}"; }}
            </script></body></html>"""
        )
    return HTMLResponse(
        f"""<!DOCTYPE html><html><body><script>
        if (window.opener) {{ window.close(); }}
        else {{ window.location.href = "{settings.frontend_url}/app/setting?calendar=connected"; }}
        </script></body></html>"""
    )
