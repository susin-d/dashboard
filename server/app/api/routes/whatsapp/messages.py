...
async def send_whatsapp_message(payload: WhatsAppMessageCreate, current_user: dict = Depends(get_current_user), database: Client = Depends(get_firestore)):
...
async def react_to_message(chat_id: str, message_id: str, payload: dict, current_user: dict = Depends(get_current_user), database: Client = Depends(get_firestore)):
...
async def star_message_endpoint(chat_id: str, message_id: str, payload: dict, current_user: dict = Depends(get_current_user), database: Client = Depends(get_firestore)):
...
async def delete_message_endpoint(chat_id: str, message_id: str, current_user: dict = Depends(get_current_user), database: Client = Depends(get_firestore)):
