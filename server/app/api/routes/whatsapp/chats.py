...
async def list_whatsapp_chats(current_user: dict = Depends(get_current_user), database: Client = Depends(get_firestore)):
...
async def get_whatsapp_messages(chat_id: str, limit: int = Query(default=50, ge=1, le=100), before: Optional[str] = Query(default=None, description="ISO timestamp to fetch messages prior to"), current_user: dict = Depends(get_current_user), database: Client = Depends(get_firestore)):
    return await WhatsAppService.get_messages(database, current_user["uid"], chat_id, limit=limit, before=before)


@router.post("/chats/{chat_id}/read")
def mark_chat_read(chat_id: str, current_user: dict = Depends(get_current_user), database: Client = Depends(get_firestore)):
    whatsapp_repo.mark_chat_as_read(database, current_user["uid"], chat_id)
    return {"status": "ok"}


@router.post("/chats/{chat_id}/summarize")
async def summarize_whatsapp_chat(chat_id: str, current_user: dict = Depends(get_current_user), database: Client = Depends(get_firestore)):
    summary = await WhatsAppService.summarize_chat(database, current_user["uid"], chat_id)
    return {"summary": summary}


@router.post("/chats/{chat_id}/summary-chat")
async def chat_about_whatsapp_summary_endpoint(chat_id: str, payload: WhatsAppSummaryChatRequest, current_user: dict = Depends(get_current_user), database: Client = Depends(get_firestore)):
...
async def generate_eve_draft(payload: dict, current_user: dict = Depends(get_current_user), database: Client = Depends(get_firestore)):
