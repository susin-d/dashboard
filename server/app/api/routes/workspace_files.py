...
def get_file_tree(
    ...
    files = workspace_files.list_tree(user["uid"])
...
def read_file(
    ...
    _require_non_serverless()
    try:
        content, size = workspace_files.read_file(user["uid"], file_path)
...
def write_file(
    ...
    _require_non_serverless()
    try:
        size = workspace_files.write_file(user["uid"], file_path, body.content, body.encoding)
    except ValueError as error:
        raise HTTPException(status_code=400, detail=str(error))
...
def delete_file(
    ...
    _require_non_serverless()
    try:
        if not workspace_files.delete_file(user["uid"], file_path):
            raise HTTPException(status_code=404, detail="File not found.")
    except ValueError as error:
        raise HTTPException(status_code=400, detail=str(error))
...
def sync_files(
    ...
    for entry in body.files:
        try:
            workspace_files.write_file(user["uid"], entry.path, entry.content, entry.encoding)
            synced += 1
