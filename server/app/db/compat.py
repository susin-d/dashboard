        ...
        with Session(self._sync_engine) as session:
            ...
            if len(path_parts) == 3 and path_parts[0] == "users" and path_parts[2] in ("settings", "integrations"):
                ...
                if not setting:
                    ...
                    setting.settings = {**(setting.settings or {}), **data} if merge else data
                session.commit()
                return
            ...
            if len(path_parts) == 3 and path_parts[0] == "users" and path_parts[2] == "whatsapp_chats":
                ...
                if not c:
                    c = WhatsAppChat(
                        ...
                        participants=_json_safe(data.get("participants") or []),
                        description=data.get("description"),
                        unread_count=int(data.get("unread_count", 0)),
                        last_message=_json_safe(data.get("last_message")),
                    ...
                    if "phone_number" in data: c.phone_number = data["phone_number"]
                    if "avatar_url" in data: c.avatar_url = data["avatar_url"]
                    if "is_group" in data: c.is_group = bool(data["is_group"])
                    if "participants" in data: c.participants = _json_safe(data["participants"])
                    if "description" in data: c.description = data["description"]
                    if "unread_count" in data: c.unread_count = int(data["unread_count"])
                    if "last_message" in data: c.last_message = _json_safe(data["last_message"])
                    if "is_pinned" in data: c.is_pinned = bool(data["is_pinned"])
                    if "is_muted" in data: c.is_muted = bool(data["is_muted"])
                    if "is_archived" in data: c.is_archived = bool(data["is_archived"])
                    if "eve_auto_reply" in data: c.eve_auto_reply = bool(data["eve_auto_reply"])
                session.commit()
                return
            ...
            if len(path_parts) == 5 and path_parts[0] == "users" and path_parts[2] == "whatsapp_chats" and path_parts[4] == "messages":
                ...
                if not m:
                    m = WhatsAppMessage(
                        ...
                        media=_json_safe(data.get("media")),
                        reply_to_message_id=data.get("reply_to_message_id"),
                        reactions=_json_safe(data.get("reactions") or []),
                    ...
                    if "content" in data: m.content = data["content"]
                    if "status" in data: m.status = data["status"]
                    if "reactions" in data: m.reactions = _json_safe(data["reactions"])
                    if "is_starred" in data: m.is_starred = bool(data["is_starred"])
                    if "is_pinned" in data: m.is_pinned = bool(data["is_pinned"])
                    if "media" in data: m.media = _json_safe(data["media"])
                session.commit()
                return
